...
`render.yaml`からDocker Web Serviceを作成し、`.env.example`に列挙した値をRender Dashboardへ登録する。`DATABASE_URL`はアプリ用、`QUEUE_DATABASE_URL`はSolid Queue用の別データベースを指す。同じNeon Project内でもデータベースを分け、Railsの`db:prepare`で両方を準備する。
...
Render Freeは停止中の`/robots.txt`へ自動的に`Disallow: /`を返し、そのリクエストでは起動しない。SEOを損なわないよう、`ops/cloudflare/seo-edge-worker`をCloudflare Workerへdeployし、`<APP_HOST>/robots.txt`の完全一致Routeだけを割り当てる。Railsの動的robotsはCloudflare未設定時のフォールバックとして残す。
