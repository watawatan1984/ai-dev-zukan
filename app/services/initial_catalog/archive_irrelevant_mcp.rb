  ...
  class ArchiveIrrelevantMcp
    ...
    def call
      ...
      if selected_uids.length < limit
        raise IncompleteSelection, "MCP selection returned #{selected_uids.length}/#{limit} resources"
      end

      reactivated_ids = reactivate_selected(selected_uids)
      archived_ids = []
      Resource.kind_mcp.unpublished.find_each do |resource|
    ...
    def reactivate_selected(selected_uids)
      Resource.kind_mcp.archived.where(external_uid: selected_uids).map do |resource|
